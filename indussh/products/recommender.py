...
class Recommender:
    products = pd.DataFrame(
        columns=['id', 'article_no', 'name', 'description', 'category'])
    recommendations = {}
    ...
    def __init__(self, products):
        ...
        for idx, row in self.products.iterrows():
            ...
            similar_items = [(cosine_similarities[idx][i],
                              self.products['article_no'][i]) for i in similar_indices]
    ...
    def recommend(self, item_article_no, num):
        ...
        # Sort the recommendations based on their score in descending order
        recs = sorted(recs, key=lambda tup: tup[0], reverse=True)
        return recs[:num]
